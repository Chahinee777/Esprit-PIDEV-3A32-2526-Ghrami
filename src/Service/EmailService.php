...
class EmailService
{
    ...
    public function sendEmail(string $toEmail, string $toName, string $subject, string $htmlContent): bool
    {
        if (!$this->isConfigured()) {
            error_log('Email provider is not configured. Set RESEND_API_KEY or MAILJET_API_KEY and MAILJET_API_SECRET in .env');
            return false;
        }

        try {
            if ($this->resendApiKey !== '') {
                return $this->sendViaResend($toEmail, $toName, $subject, $htmlContent);
            }

            return $this->sendViaMailjet($toEmail, $toName, $subject, $htmlContent);
        } catch (\Exception $e) {
    ...
    {
        return $this->resendApiKey !== '' || (!empty($this->apiKey) && !empty($this->apiSecret));
    }

    private function sendViaMailjet(string $toEmail, string $toName, string $subject, string $htmlContent): bool
    {
        $payload = [
            'Messages' => [
                [
    ...
    private function sendViaResend(string $toEmail, string $toName, string $subject, string $htmlContent): bool
    {
        $recipient = $toName !== '' ? sprintf('%s <%s>', $toName, $toEmail) : $toEmail;

        $response = $this->httpClient->request('POST', self::RESEND_URL, [
            'headers' => [
        ...
        error_log('Resend error (status ' . $statusCode . '): ' . $response->getContent(false));
