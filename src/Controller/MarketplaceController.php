...
final class MarketplaceController extends AbstractController
{
    ...
    public function index(Request $request, MarketplaceService $marketplaceService, EntityManagerInterface $em): Response
    {
        ...
        if ($userId) {
            $provider = $em->getConnection()->fetchAssociative(
                'SELECT provider_id, is_verified FROM class_providers WHERE user_id = :userId',
                ['userId' => $userId]
            );
            if ($provider) {
                $isInstructor       = (bool) $provider['is_verified'];
        ...
        $classes = $marketplaceService->listClasses(
            ...
            $request->query->get('max_price') !== null ? (float) $request->query->get('max_price') : null,
            $request->query->get('q'),
            $userId
        );
        ...
        switch ($sort) {
            case 'price_asc':  usort($classes, fn ($a, $b) => $a['price'] <=> $b['price']); break;
            case 'price_desc': usort($classes, fn ($a, $b) => $b['price'] <=> $a['price']); break;
            case 'rating':     usort($classes, fn ($a, $b) => ($b['rating'] ?? 0) <=> ($a['rating'] ?? 0)); break;
            case 'newest':     usort($classes, fn ($a, $b) => strtotime($b['created_at'] ?? '2000-01-01') - strtotime($a['created_at'] ?? '2000-01-01')); break;
            default:           usort($classes, fn ($a, $b) => ($b['participants'] ?? 0) <=> ($a['participants'] ?? 0));
        }
